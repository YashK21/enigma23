...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
	...
	{
		$result=mysqli_query($conn,"SELECT * from levels where level='6';"); 
	if($result and mysqli_num_rows($result)==1)
	{
		...
		if($row_ans["ans"]==md5($_POST["ans"]))
		{
			...
			if($_SESSION['level']>$row1['level']){
				$sql_up="update users set level='7',up_time='".date("Y-m-d H:i:s")."' where reg_no='".$row['reg_no']."'";
...
<body>
    ...
    <section class="container mt-5 pt-5 mb-3 text-white text-center">
        <div class="row">
            ...
            <div class="row">
            <div class="col-lg-12">
            <img class="img-fluid" src="./assets/img/lvls/6asasa9asas12sasa.jpg" style="border-radius:25px;padding:0px;">
            </div>
            </div>
            <br/>
            <div class="row">
          ...
          $limit=(-$tt+$now)/3600;
					...
					$result=mysqli_query($conn,"SELECT * from hints where level_h='6' order by hint_n");
					if($result)
