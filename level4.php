	...
	if($result and mysqli_num_rows($result)==1)
	{
		...
		if($row_ans["ans"]==md5($_POST["ans"]))
		{
			...
			if($_SESSION['level']>$row1['level']){
				$sql_up="update users set level='5',up_time='".date("Y-m-d H:i:s")."' where reg_no='".$row['reg_no']."'";
...
<body>
    ...
    <section class="container mt-5 pt-5 mb-3 text-white text-center">
        <div class="row">
            ...
            <div class="row">
            <div class="col-lg-12">
            <img class="img-fluid" src="./assets/img/lvls/4abcasb9asabsaa0.jpg" style="border-radius:25px;padding:0px;">
            </div>
            </div>
            <br/>
            <div class="row">
          ...
          $limit=(-$tt+$now)/3600;
					...
					$result=mysqli_query($conn,"SELECT * from hints where level_h='4' order by hint_n");
					if($result)
                        $hint=mysqli_num_rows($result);
					if($hint>=1 and $limit> ($_SESSION['t1']*2))
					{
            ...
            if($hint>=2 and $limit> ($_SESSION['t1']*4))
            {
                $row=mysqli_fetch_array($result,MYSQLI_ASSOC);
                echo '<div class="row">
                    <div class="col-sm-8 col-lg-offset-4">
            ...
            if($hint>=3 and $limit> ($_SESSION['t1']*10))
            {
                $row=mysqli_fetch_array($result,MYSQLI_ASSOC);
                echo '<div class="row">
                    <div class="col-sm-8 col-lg-offset-4">
